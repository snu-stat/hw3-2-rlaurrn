...
RUN conda create -n r-reticulate python=3.10 -y && \
    conda install -n r-reticulate -c conda-forge pip numpy pandas matplotlib polars plotnine statsmodels -y && \
    conda run -n r-reticulate python -m pip install --no-cache-dir pylahman
# 추가로 필요한 패키지 설치
